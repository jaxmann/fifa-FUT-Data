...
with open('FutBinCards19.csv', 'w') as f:
    for key, value in fifa.items():
        print('Doing ' + value)
        FutBin = requests.get('https://www.futbin.com/' + key + '/players', headers={'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/71.0.3578.80 Chrome/71.0.3578.80 Safari/537.36'})
        ...
        for page in range(1, int(TotalPages) + 1):
            print(page)
            FutBin = requests.get('https://www.futbin.com/' + key + '/players?page=' + str(page), headers={'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/71.0.3578.80 Chrome/71.0.3578.80 Safari/537.36'})
            bs = BeautifulSoup(FutBin.text, 'html.parser')
            ...
            for idx, webpage in enumerate(webpages):
                d = {}
                json_data = ''
                profile = requests.get(webpage, headers={'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/71.0.3578.80 Chrome/71.0.3578.80 Safari/537.36'})
                ...
                # print(cardDetails[idx])
                info = bs.find('div', {'id': 'info_content'})
                # print('info', info)
                if info:
                    # print(dict(zip([str(i.text).replace('Name', 'Fullname').strip() for i in info.findAll('th')],
                    #          [str(i.text).strip() for i in info.findAll('td')])))
                ...
                vals = []
                for v in l:
                    if v[0] == 'League' or v[0] == 'Club' or v[0] == 'Fullname' or v[0] == 'Nation':
                        vals.append(v[1])
                vals.append(cardDetails[idx][2])
                vals.append(cardDetails[idx][3])
                vals.append(cardDetails[idx][4])
